...
function parseSObjectStub(filePath) {
  ...
  return { objectName, fields, childRelationships };
}
...
const childRelationshipByObjectAndField = new Map();
for (const entry of entries) {
  for (const relationship of entry.childRelationships) {
    const key = `${relationship.childObject}.${relationship.fieldName}`;
    if (!childRelationshipByObjectAndField.has(key)) {
      childRelationshipByObjectAndField.set(key, relationship.relationshipName);
    }
  }
}
...
for (const entry of entries) {
  ...
  for (const field of entry.fields) {
    field.childRelationshipName = childRelationshipByObjectAndField.get(`${entry.objectName}.${field.name}`) || "";
