...
function relationshipLiteral(relationship) {
  ...
  const parentRelationship = relationshipName(relationship.fieldName);
...
for (const entry of entries) {
  ...
  for (const field of entry.fields) {
    const childRelationshipNames = childRelationshipsByObjectAndField.get(`${entry.objectName}.${field.name}`);
