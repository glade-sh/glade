	...
	for _, typ := range index.Types {
		...
		for _, member := range typ.Members {
			...
			case apexast.DeclarationMethod, apexast.DeclarationConstructor, apexast.DeclarationInitializer:
				source, ok := readSemaSource(typ.File, index.Project.Namespace, sources)
				if !ok {
					continue
				}
...
func readSemaSource(path, namespace string, cache map[string]string) (string, bool) {
	if path == "" {
		return "", false
	}
	key := path + "\x00" + namespace
