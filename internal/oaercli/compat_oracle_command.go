...
func runCompatOracle(ctx context.Context, args []string, w io.Writer) error {
	...
	case "run-oaer":
		return runCompatOracleRunOAER(ctx, args[1:], w)
	case "diff":
...
Usage:
  oaer compat oracle <subcommand> [flags]
...
Subcommands:
  ...
  run-oaer        Run OAER oracle test shard.
  diff            Diff Salesforce and OAER observations.
...
func runCompatOracleRunOAER(ctx context.Context, args []string, w io.Writer) error {
	...
	runDir := filepath.Join(runsDir, runID)
	obsPath := filepath.Join(runDir, "oaer", "observations", fmt.Sprintf("shard-%03d.json", shard))
	if err == nil {
		err = oracle.WriteJSON(obsPath, runs)
	}
	status := "ok"
	...
	_ = oracle.AppendLedger(filepath.Join(runDir, "ledger.jsonl"), oracle.LedgerRow{RunID: runID, Step: "run-oaer", Shard: shard, Status: status, Message: msg})
	if err != nil {
		return err
	}
...
func runCompatOracleDiff(args []string, w io.Writer) error {
	...
	oaerPath := filepath.Join(runDir, "oaer", "observations", fmt.Sprintf("shard-%03d.json", shard))
	sfRuns, err := oracle.ReadRuns(sfPath)
	if err != nil {
		return err
	}
	localRuns, err := oracle.ReadRuns(oaerPath)
	if err != nil {
		return err
	}
...
func runCompatOraclePromote(args []string, w io.Writer) error {
	...
	for _, src := range []string{"salesforce", "oaer", "diffs", "ledger.jsonl"} {
