...
var mvpFeatures = []Feature{
	...
	{ID: "dml.apex", Area: "Data runtime", Name: "Apex DML statements and Database methods", Status: StatusPartial, Required: true, Notes: "Apex insert/update/delete/upsert/undelete syntax and Database.insert/update/delete/upsert/undelete allOrNone paths now call the DML engine, return SaveResult/UpsertResult objects with statusCode, message, fields arrays, isCreated, set Ids, roll back allOrNone failures, soft-delete and undelete records, match implicit and explicit external-ID upserts, reject ID/object mismatches, enforce unique fields, validate lookup references, and cascade soft-delete children from relationship metadata. Merge, validation-rule formulas, and full Salesforce status-code parity remain incomplete."},
