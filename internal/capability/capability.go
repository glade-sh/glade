...
var mvpFeatures = []Feature{
	...
	{ID: "server.local-api", Area: "Local API server", Name: "Salesforce-shaped local API with CRUD/query/executeAnonymous", Status: StatusPartial, Required: true, Notes: "Version/root discovery, SObject CRUD with normal REST JSON payloads and explicit nulls, describe/recent, query/queryAll with REST-shaped attributes and soft-deleted row visibility, limits, OAuth userinfo/id stubs with local user selection, Tooling executeAnonymous GET/POST success and failure shapes, supported local-object Tooling queries, Composite sObject insert with referenceId/partial success/allOrNone rollback, Salesforce-shaped error arrays, SQLite persistence, and GLADE fixture/scoped reset endpoints are wired with black-box compatibility coverage. Full auth, unmodeled Tooling objects, Composite batch/Graph, Bulk API, layout metadata, and broader REST resources remain incomplete or explicitly unsupported."},
