...
var mvpFeatures = []Feature{
	...
	{ID: "lsp.command", Area: "Developer experience", Name: "oaer lsp core editor features", Status: StatusPartial, Required: true, Notes: "oaer lsp now runs a stdio LSP transport with initialize, diagnostics, incremental text document sync through didOpen/didChange/didClose overlays, document/workspace symbols, semantic tokens, definition, references, prepare-rename/rename workspace edits, hover, and completion for Apex types, members, SObjects, fields, and keywords. Full check-parity diagnostics and deeper context-aware completion remain incomplete."},
