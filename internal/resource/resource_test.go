...
func TestLoadProjectRemapsDependencyResourceNamespaces(t *testing.T) {
	...
	resourcePath := filepath.Join(root, "force-app/main/staticresources/BasePkg__BillingAssets.resource")
	resourceMeta := filepath.Join(root, "force-app/main/staticresources/BasePkg__BillingAssets.resource-meta.xml")
	contentAssetPath := filepath.Join(root, "force-app/main/contentassets/BasePkg__BillingLogo.asset")
	tabPath := filepath.Join(root, "force-app/main/tabs/BasePkg__Billing__c.tab-meta.xml")
	quickActionPath := filepath.Join(root, "force-app/main/quickActions/BasePkg__Billing__c.BasePkg__Create.quickAction-meta.xml")
	dataCategoryPath := filepath.Join(root, "force-app/main/datacategorygroups/Billing.dataCategoryGroup-meta.xml")
	namedCredentialPath := filepath.Join(root, "force-app/main/namedCredentials/BasePkg__BillingEndpoint.namedCredential-meta.xml")
	writeFile(t, labelPath, `<CustomLabels xmlns="http://soap.sforce.com/2006/04/metadata"><labels><fullName>BasePkg__Billing_Error</fullName><language>en_US</language><protected>false</protected><shortDescription>Billing Error</shortDescription><value>Bad bill</value></labels></CustomLabels>`)
	...
	writeFile(t, quickActionPath, `<QuickAction xmlns="http://soap.sforce.com/2006/04/metadata"><label>Create</label><type>Create</type><targetObject>BasePkg__Billing__c</targetObject><predefinedFieldValues><field>BasePkg__Code__c</field><value>42</value></predefinedFieldValues></QuickAction>`)
	writeFile(t, dataCategoryPath, `<DataCategoryGroup xmlns="http://soap.sforce.com/2006/04/metadata"><label>Billing</label><objectUsage><object>BasePkg__Billing__c</object></objectUsage><dataCategory><name>All</name><label>All</label></dataCategory></DataCategoryGroup>`)
	...
	if len(registry.Labels) != 1 || registry.Labels[0].Name != "stagepkg__Billing_Error" || registry.Labels[0].Namespace != "stagepkg" {
		t.Fatalf("labels = %#v", registry.Labels)
	}
	if len(registry.StaticResources) != 1 || registry.StaticResources[0].Name != "stagepkg__BillingAssets" || registry.StaticResources[0].NamespacePrefix != "stagepkg" {
		t.Fatalf("static resources = %#v", registry.StaticResources)
	}
	if len(registry.ContentAssets) != 1 || registry.ContentAssets[0].Name != "stagepkg__BillingLogo" || registry.ContentAssets[0].URL != ContentAssetURL("stagepkg__BillingLogo") {
		t.Fatalf("content assets = %#v", registry.ContentAssets)
	}
	if len(registry.Tabs) != 1 || registry.Tabs[0].Name != "stagepkg__Billing__c" || registry.Tabs[0].SObjectName != "stagepkg__Billing__c" {
		t.Fatalf("tabs = %#v", registry.Tabs)
	}
	if len(registry.QuickActions) != 1 || registry.QuickActions[0].Name != "stagepkg__Billing__c.stagepkg__Create" || registry.QuickActions[0].TargetObject != "stagepkg__Billing__c" || len(registry.QuickActions[0].PredefinedFieldValues) != 1 || registry.QuickActions[0].PredefinedFieldValues[0].Field != "stagepkg__Code__c" {
		t.Fatalf("quick actions = %#v", registry.QuickActions)
	}
	if len(registry.DataCategoryGroups) != 1 || registry.DataCategoryGroups[0].SObjectName != "stagepkg__Billing__c" {
