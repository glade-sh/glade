...
func TestCollectSOSLUsesFromStaticBracketQuery(t *testing.T) {
	...
	assertSOSLUseAt(t, uses, codeintel.SObjectID("Account"), codeintel.UseQuery, "Account", 3, 69)
	assertSOSLUseAt(t, uses, codeintel.SObjectFieldID("Account", "Id"), codeintel.UseQuery, "Id", 3, 77)
	assertSOSLUseAt(t, uses, codeintel.SObjectFieldID("Account", "Name"), codeintel.UseQuery, "Name", 3, 81)
	assertSOSLUseAt(t, uses, codeintel.SObjectID("Contact"), codeintel.UseQuery, "Contact", 3, 88)
	assertSOSLUseAt(t, uses, codeintel.SObjectFieldID("Contact", "Id"), codeintel.UseQuery, "Id", 3, 96)
}
...
func assertSOSLUseAt(t *testing.T, uses []codeintel.Use, id codeintel.SymbolID, kind codeintel.UseKind, name string, line, column int) {
	t.Helper()
