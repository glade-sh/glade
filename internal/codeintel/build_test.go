...
func TestBuildIncludesSOSLUsesFromIndexedApexFiles(t *testing.T) {
	...
	source := "public class Searcher {\n" +
		...
		"  }\n" +
		"}\n"
	writeTestFile(t, filepath.Join(root, classPath), source)
	...
	assertUse(t, graph.Uses, codeintel.ApexTypeID("", "Searcher"), codeintel.UseDeclaration, "Searcher", 1, 1)
	assertUse(t, graph.Uses, codeintel.SObjectID("Account"), codeintel.UseQuery, "Account", 3, 69)
	assertUse(t, graph.Uses, codeintel.SObjectFieldID("Account", "Id"), codeintel.UseQuery, "Id", 3, 77)
	assertUse(t, graph.Uses, codeintel.SObjectFieldID("Account", "Name"), codeintel.UseQuery, "Name", 3, 81)
}
