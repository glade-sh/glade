...
func TestLoadProject(t *testing.T) {
	...
	s, err := LoadProject(project.Project{ObjectFiles: []string{objectPath}, FieldFiles: []string{fieldPath, formulaFieldPath}, RecordTypeFiles: []string{recordTypePath, lowercaseRecordTypePath}, ValidationRuleFiles: []string{validationRulePath}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(s.Objects[0].Fields) != 2 || len(s.Objects[0].Fields[0].ReferenceTo) != 2 || s.Objects[0].Fields[0].ReferenceTo[0] != "Thing__c" || s.Objects[0].Fields[0].ReferenceTo[1] != "Account" {
