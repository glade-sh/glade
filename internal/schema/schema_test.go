...
func TestLoadProject(t *testing.T) {
	...
	s, err := LoadProject(project.Project{ObjectFiles: []string{objectPath}, FieldFiles: []string{fieldPath, formulaFieldPath, encryptedFieldPath}, RecordTypeFiles: []string{recordTypePath, lowercaseRecordTypePath}, ValidationRuleFiles: []string{validationRulePath}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(s.Objects[0].Fields) != 3 || len(s.Objects[0].Fields[0].ReferenceTo) != 2 || s.Objects[0].Fields[0].ReferenceTo[0] != "Thing__c" || s.Objects[0].Fields[0].ReferenceTo[1] != "Account" {
		t.Fatalf("fields = %#v", s.Objects[0].Fields)
	}
	if got := s.Objects[0].Fields[1].Formula; got != "1 + 1" {
		t.Fatalf("formula = %q", got)
	}
	if !s.Objects[0].Fields[2].Encrypted {
		t.Fatalf("encrypted field = %#v", s.Objects[0].Fields[2])
	}
