...
func runProbe(ctx context.Context, args []string, w io.Writer) error {
	...
	if len(args) == 0 {
		return fmt.Errorf("usage: glade probe <subcommand> [args...]\n  subcommands: org, local, deploy, summarize")
	}
...
func runProbeSummarize(ctx context.Context, args []string, w io.Writer) error {
	...
	if len(args) == 0 {
		return fmt.Errorf("usage: glade probe summarize <gap-report.json> [--json] [--top-stub]")
	}
	reportPath := ""
	asJSON := false
	topStub := false
...
func runProbeOrg(ctx context.Context, args []string, w io.Writer) error {
	...
	// Load glade.yml features if present
	var features []string
