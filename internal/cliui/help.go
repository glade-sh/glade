...
var commandReferences = []CommandHelp{
	...
	{
		Name:        "debug",
		Description: "Parse, profile, explain, analyze for editors, and synthesize from Salesforce debug logs.",
		Usage:       []string{"glade debug parse --log <path> [--json]", "glade debug profile --log <path> [--json] [--format text|markdown]", "glade debug explain --log <path> [--project <root>] [--min-confidence <n>] [--json]", "glade debug editor --log <path> [--project <root>] [--min-confidence <n>] [--max-candidates <n>] [--json]", "glade debug repro --log <path> [--project <root>] [--min-confidence <n>]", "glade debug replay --log <path> [--project <root>] [--min-confidence <n>] [--entry-index <n>] [--json]"},
		Subcommands: []SubcommandHelp{
			...
			{Name: "editor", Description: "Build Apex log editor navigation, folding, symbols, hovers, and diagnostics."},
		...
		Flags: []FlagHelp{
			...
			{Name: "--project", Value: "<root>", Description: "Project root for explain, editor, repro, and replay."},
			{Name: "--min-confidence", Value: "<n>", Description: "Minimum confidence for explain, editor, repro, and replay."},
			{Name: "--max-candidates", Value: "<n>", Description: "Maximum source candidates for editor analysis."},
			{Name: "--entry-index", Value: "<n>", Description: "Zero-based log entry index for replay from a source-backed frame."},
		...
		Examples: []string{"glade debug profile --log apex.log", "glade debug explain --log apex.log --project .", "glade debug editor --log apex.log --project . --json", "glade debug replay --log apex.log --project . --json"},
	},
	{
		Name:        "editor",
	...
	},
	{
		...
		Usage:       []string{"glade tui [--project <root>] [--env <name>|--db <path>] [--view <project|tests|data|plugins>] [--target-org <alias>] [--object <Object>]"},
		Flags: []FlagHelp{
			{Name: "--project", Value: "<root>", Description: "Project root. Defaults to current directory."},
			{Name: "--env", Value: "<name>", Description: "Project local data environment for the data board. Defaults to dev."},
			{Name: "--db", Value: "<path>", Description: "Explicit SQLite database path override for DB actions."},
		...
		Examples: []string{"glade tui --project .", "glade tui --project . --view tests", "glade tui --project . --view data", "glade tui --project . --env qa --view data", "glade tui --project . --view data --target-org devhub --object Account"},
	},
	{
		Name:        "dev",
	...
	{
		Name:        "package",
		Description: "Build, inspect, validate, diff, and capture managed package artifacts.",
		Usage: []string{
			...
			"glade package capture --target-org <alias> --namespace <namespace> --output <artifact> [--config-snippet]",
		},
		Subcommands: []SubcommandHelp{
			...
			{Name: "capture", Description: "Bridge to @glade/orgpackage for org-backed artifact capture."},
		},
		Flags: []FlagHelp{
			{Name: "--project", Value: "<root>", Description: "Project root. Defaults to current directory."},
			...
			{Name: "--target-org", Value: "<alias>", Description: "Org alias passed to orgpackage capture."},
			{Name: "--config-snippet", Description: "Ask orgpackage capture to print a glade.yml dependency snippet."},
		...
		Notes: []string{
			"`glade package capture` requires the @glade/orgpackage plugin. Install it with `glade plugins install @glade/orgpackage` or link a local plugin executable.",
			"Capture dispatches to `glade orgpackage capture`; base Glade keeps artifact loading, validation, diffing, and local runtime behavior.",
		},
		Examples: []string{
	...
	},
	{
		...
		Flags: []FlagHelp{
			{Name: "--addr", Value: "<host:port>", Description: "Bind address. Defaults to 127.0.0.1:8080. Network-reachable binds require GLADE_SERVER_PUBLIC=1."},
	...
	{
		Name:        "org",
		Description: "Create, inspect, start, and authenticate local Glade org targets.",
		Usage: []string{
			"glade org create <alias> [--project <root>] [--db <path>] [--addr <host:port>] [--json]",
			"glade org list [--project <root>] [--json]",
			"glade org status <alias> [--project <root>] [--json]",
			"glade org start <alias> [--project <root>]",
			"glade org auth <alias> [--project <root>] [--sf-config-dir <path>] [--print]",
		},
		Subcommands: []SubcommandHelp{
			{Name: "create", Description: "Create a local org config and initialize its SQLite database."},
			{Name: "list", Description: "List configured local orgs."},
			{Name: "status", Description: "Check the local org userinfo endpoint."},
			{Name: "start", Description: "Start the local Salesforce-compatible API for an org."},
			{Name: "auth", Description: "Log the Salesforce CLI into the local org target."},
		},
		Flags: []FlagHelp{
			{Name: "--project", Value: "<root>", Description: "Project root. Defaults to current directory."},
			{Name: "--db", Value: "<path>", Description: "Persistent local database path. Defaults to .glade/orgs/<alias>.sqlite."},
			{Name: "--addr", Value: "<host:port>", Description: "Local server address. Defaults to 127.0.0.1:17911 and scans upward. Network-reachable binds require GLADE_SERVER_PUBLIC=1."},
	...
	},
	{
		...
		Flags: []FlagHelp{
			{Name: "--addr", Value: "<host:port>", Description: "Bind address. Defaults to 127.0.0.1:1789. Network-reachable binds require GLADE_SERVER_PUBLIC=1."},
			{Name: "--db", Value: "<path>", Description: "Persistent local database."},
	...
	},
	{
		...
		Usage: []string{
			...
			"glade db import sf [--target-org <alias>] [--project <root>] [--env <name>|--db <path>] [--object <Object>|--query <soql>] [--fields Id,Name] [--limit <n>] [--json]",
			"glade db import sf [--target-org <alias>] --list-objects [--category all|standard|custom] [--json]",
			"glade db reset [--project <root>] [--env <name>|--db <path>] [--json]",
			"glade db export [--project <root>] [--env <name>|--db <path>]",
			"glade db inspect [--project <root>] [--env <name>|--db <path>] [--json]",
			"glade db query [--project <root>] [--env <name>|--db <path>] --json [--limit <n>] [--query-all] <soql>",
		...
		Flags: []FlagHelp{
			{Name: "--env", Value: "<name>", Description: "Project local data environment. Defaults to dev at .glade/envs/dev.sqlite."},
			...
			{Name: "--addr", Value: "<host:port>", Description: "Address for glade db ui. Defaults to 127.0.0.1:0. Network-reachable binds require GLADE_SERVER_PUBLIC=1."},
		...
		Examples: []string{
			...
			`glade db query --project . --json "SELECT Id, Name FROM FileRow__c"`,
			"glade db describe --project . --json FileRow__c",
		},
	},
	{
		Name:        "completion",
