...
| Local LWC shell and Visualforce Lightning Out | Preview feature. The printed base URL at `/` and `/lwc` open a local workbench with a filterable available-LWC catalog, draft page composer, generated routes, and context diagnostics. `/lwc/preview/component`, URL-addressable, record page, app page, home page, tab, record action, utility bar, Flow screen, Flow action, community, Visualforce-backed tab redirects, and `/apex/<PageName>` Lightning Out hosts use the shared local LWC runtime. Apex wire, selected LDS/UI API shims including batch records, `lightning/uiObjectInfoApi` object info and picklists, `lightning/uiRelatedListApi` child rows, create defaults, and `lightning/uiLayoutApi` `getLayout`, DML-backed local record mutations, datatable and record form renderers, schema tokens, labels, static resource subpaths, `CurrentPageReference`, `NavigationMixin`, LMS, resource loading, toast event paths, Flow screen events, EMP in-page events, console workspace tab modeling, and all public module names exposed by `lightning-base-components@1.28.19-alpha` work with local-data limits. |
