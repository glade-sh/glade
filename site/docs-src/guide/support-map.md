...
| Local LWC shell and Visualforce Lightning Out | Preview feature. The printed base URL at `/` and `/lwc` opens a local LWC home page with formal tabs and discovered routes. `/lwc/builder` opens the draft page composer with a filterable available-LWC catalog, drag-and-drop placement, layout switching, seeded record context, and context diagnostics. `/lwc/preview/component`, URL-addressable, record page, app page, home page, tab, record action, utility bar, Flow screen, Flow action, community page, direct community component, Visualforce-backed tab redirects, and `/apex/<PageName>` Lightning Out hosts use the shared local LWC runtime. Apex wire, selected LDS/UI API shims including batch records, `lightning/uiObjectInfoApi` object info and picklists, `lightning/uiRelatedListApi` child rows, create defaults, `lightning/uiLayoutApi` `getLayout`, DML-backed local record mutations, datatable and record form renderers, schema tokens, labels, static resource subpaths, `CurrentPageReference`, `NavigationMixin`, LMS, resource loading, toast event paths, Flow screen events, `lightning/actions`, `lightning/refresh`, overlay/config/page-reference helper shims, EMP in-page events, console workspace tab modeling, packaged SLDS 2 and classic SLDS assets, and all public module names exposed by `lightning-base-components@1.28.19-alpha` work with local-data limits. |
...
| Exact hosted Lightning Experience behavior | The local LWC shell does not promise Salesforce-hosted app chrome, permissions, full UI API, every `lightning-*` base component edge, exact SLDS fidelity, Flow Builder behavior, live EMP streaming, or every Lightning Out edge. Console workspace APIs have a local tab model for development; exact hosted console behavior remains a Salesforce check. The package-exposed base-component module surface resolves locally with practical shims, including common button/card/layout/formatted-number contracts, with a small source-backed allowlist for simple base components. |
