...
test('routes classify every published page and metadata is generated per route', () => {
  ...
  assert.match(config, /resolvedDescription\(pageData/)
  assert.match(config, /description: resolvedDescription\(pageData\)/)
  assert.match(config, /const description = resolvedDescription\(ctx\.pageData\)/)
