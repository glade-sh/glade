...
test("home page balances a modern product demo with honest proof", () => {
  ...
  assert.match(index, /Unknown type "Invoice__c"/);
  assert.match(index, /1 type checked · 0 org calls · exit code 1/);
  ...
  assert.match(index, /1 diagnostic \(1 error\)/);
  ...
  assert.match(index, /Source, issues, releases, and roadmap\./);
...
test("home script delegates controls after VitePress hydration", () => {
  ...
  assert.match(homeScript, /glade debug profile --log internal\/debuglog\/testdata\/subscriber\.log/);
  assert.match(homeScript, /# glade profile/);
  assert.match(homeScript, /Events: 4/);
  assert.match(homeScript, /SOQL: 1 queries \/ 1 rows/);
  ...
  assert.match(homeScript, /"file": "\/private\/tmp\/glade-home-account-field\/force-app\/main\/default\/classes\/AccountService\.cls"/);
  ...
  assert.match(homeScript, /glade exec --debug-log - .*System\.debug\('local'\);/);
  assert.match(homeScript, /USER_DEBUG\|\[1\]\|DEBUG\|local/);
...
test("brand guide page documents the settled identity", () => {
  ...
  assert.match(brandGuide, /Apex feedback before you deploy\./);
  assert.match(brandGuide, /Host Signal/);
  assert.match(brandGuide, /Host Grotesk/);
  assert.match(brandGuide, /Monaspace Argon/);
  assert.doesNotMatch(brandGuide, /Apfel Air|Apfel Grotezk|Commit Mono|Fraunces Variable|Newsreader|Mona Sans|Monaspace Neon|JetBrains Mono|Source Serif|Literata|Lora/);
  assert.match(brandGuide, /#9BE870/);
  assert.match(brandGuide, /#B7FF8A/);
  assert.match(brandGuide, /#070B0D/);
  assert.match(brandGuide, /#10191E/);
  assert.match(brandGuide, /#F5C95F/);
  assert.match(brandGuide, /#FF6B61/);
  assert.match(brandGuide, /38;2;155;232;112/);
  assert.match(brandGuide, /38;2;183;255;138/);
  assert.match(brandGuide, /Give the mark room to read as terrain, not texture/);
  assert.match(brandGuide, /glade-mark\.svg/);
  assert.match(brandGuide, /--text: #F3F7F5/);
  assert.match(brandGuide, /--warning: #F5C95F/);
  ...
  assert.doesNotMatch(css, /\.brand-font-newsreader|\.brand-font-source-serif/);
