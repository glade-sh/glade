...
test("home page uses a polished local loop proof without experiment tabs", () => {
  ...
  assert.match(index, /<h1>Run Apex locally before you deploy\.<\/h1>/);
  ...
  assert.match(index, /class="home-lead"[\s\S]*Local Apex checks, tests, snippets, SOQL, DML, logs, and API routes on your machine\./);
  assert.match(index, /href="\/guide\/workbench"[^>]*data-demo-link[^>]*>Explore support<\/a>/);
  assert.match(index, /href="\/guide\/installation"[\s\S]*Install Glade/);
  assert.match(index, /href="\/guide\/support-map"[\s\S]*Check support/);
  assert.match(index, /No org login for supported local work\./);
  assert.match(index, /Salesforce stays in the path for hosted services\./);
  ...
  assert.match(index, /local data/);
  assert.match(index, /no org wait/);
  ...
  assert.match(index, /One local runtime for the daily Apex loop\./);
  assert.match(index, /check source · run tests · execute snippets · query local data · profile logs · serve local APIs · emit JSON/);
  assert.match(index, /Local work/);
  assert.match(index, /Support boundary/);
  assert.match(index, /Support showcase/);
  assert.match(index, /Visualforce pages and the LWC local shell remain preview features\./);
  assert.match(index, /Works locally where Glade has evidence\. Hosted Salesforce behavior stays marked\./);
  ...
  assert.match(config, /\{ text: 'Showcase', link: '\/guide\/workbench' \}/);
...
test("site copy is task-first and names local support plainly", () => {
  assert.match(index, /<h1>Run Apex locally before you deploy\.<\/h1>/);
  assert.match(index, /Local Apex checks, tests, snippets, SOQL, DML, logs, and API routes on your machine\./);
  assert.match(index, /Explore support/);
  assert.match(index, /No org login for supported local work\./);
  assert.match(index, /Salesforce stays in the path for hosted services\./);
  ...
  assert.match(workbench, /Use the editor as a live support map: type a dot, read the label, and see the boundary before you depend on an API\./);
  assert.match(workbench, /Support cards/);
  assert.match(workbench, /Workflow gallery/);
  assert.match(workbench, /Local result/);
  assert.match(codeMirrorWorkbench, /Try support-backed autocomplete\./);
  assert.match(codeMirrorWorkbench, /Type a dot after/);
  assert.match(config, /description: 'Run Apex checks, tests, snippets, logs, and local API routes before you deploy\.'/);
  assert.match(config, /\{ text: 'Support', link: '\/guide\/support-map' \}/);
  assert.match(config, /\{ text: 'Showcase', link: '\/guide\/workbench' \}/);
...
test("home local loop styles stay quiet and responsive", () => {
  ...
  assert.match(css, /\.home-hero-shell\s*\{[\s\S]*grid-template-columns: minmax\(360px, 0\.8fr\) minmax\(640px, 1\.2fr\);/);
  assert.match(css, /\.home-hero-shell\s*\{[\s\S]*padding: 64px 0 20px;/);
  ...
  assert.match(css, /\.home-loop-visual\s*\{[\s\S]*min-height: 520px;[\s\S]*overflow: hidden;/);
  assert.match(css, /\.home-loop-terminal\s*\{[\s\S]*height: 234px;[\s\S]*overflow: hidden;/);
  assert.match(css, /\.home-loop-terminal-body\s*\{[\s\S]*position: relative;[\s\S]*height: calc\(100% - 38px\);/);
  assert.match(css, /\.home-loop-state\s*\{[\s\S]*position: absolute;[\s\S]*inset: 0;[\s\S]*grid-template-rows: 34px minmax\(78px, 1fr\) 50px;[\s\S]*opacity: 0;/);
  assert.match(css, /\.home-loop-state\.active\s*\{[\s\S]*opacity: 1;[\s\S]*transform: translateY\(0\);/);
  assert.match(css, /\.home-loop-result\s*\{[\s\S]*min-height: 72px;/);
  assert.match(css, /\.home-loop-stage\s*\{[\s\S]*height: 214px;/);
  ...
  assert.match(css, /@media \(max-width: 640px\)\s*\{[\s\S]*\.home-loop-visual\s*\{[\s\S]*min-height: 548px;/);
  assert.match(css, /@media \(max-width: 640px\)\s*\{[\s\S]*\.home-loop-terminal\s*\{[\s\S]*height: 260px;/);
  assert.match(css, /@media \(max-width: 640px\)\s*\{[\s\S]*\.home-loop-state\s*\{[\s\S]*grid-template-rows: 34px minmax\(78px, 1fr\) 50px;/);
...
test("support showcase page carries autocomplete and workflow examples", () => {
  assert.match(workbench, /^# Support showcase/m);
  assert.match(workbench, /data-support-showcase/);
  assert.match(workbench, /class="support-showcase-cards"/);
...
test("theme uses the Host Signal design direction", () => {
  ...
  assert.match(config, /title: 'Glade — Local Apex Support Showcase'/);
  assert.match(config, /description: 'Run Apex checks, tests, snippets, logs, and local API routes before you deploy\.'/);
  assert.match(config, /\['meta', \{ property: 'og:title', content: 'Glade — Run Apex locally before you deploy' \}\]/);
  assert.match(config, /\['meta', \{ property: 'og:description', content: 'Local Apex tooling for checks, tests, snippets, debug-log profiling, and copyable CI commands\.' \}\]/);
  assert.match(config, /\['meta', \{ property: 'og:type', content: 'website' \}\]/);
  assert.match(config, /Run Apex locally before you deploy/);
  assert.match(index, /<h1>Run Apex locally before you deploy\.<\/h1>/);
  assert.match(config, /Local Apex Support Showcase/);
  assert.match(config, /siteTitle: 'Glade'/);
  assert.match(config, /local API routes/);
  ...
  assert.match(config, /text: 'Support'/);
