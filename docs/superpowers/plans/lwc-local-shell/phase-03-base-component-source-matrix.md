...
| `@salesforce-ux/design-system` | BSD-3-Clause package. Some icons and images use separate CC BY-ND terms. | CSS assets fit the existing local SLDS lane. No base-component behavior. | Keep using the local SLDS-shaped stylesheet unless a future styling phase vendors selected CSS with notices. |
| `@salesforce-ux/design-system-2` | Includes Salesforce terms. | Useful shell and styling reference. No LWC behavior. | Reference only. |
