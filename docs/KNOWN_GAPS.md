...
- Status: `partial`
- Gap: CRUD/query/queryAll including bounded local batchSize pagination and process-local queryMore locators, SObject discovery and object-resource metadata with stable describe/recent URL shapes, describe/recent, record GET attributes/url payloads, explicit layout unsupported responses, compact layout stubs, limits, deterministic local OAuth userinfo/id stubs with X-OAER-User-Id and Authorization: Bearer <userId> selection, Tooling executeAnonymous with selected user context, Tooling query, explicit unsupported Tooling sObject/completions routes, explicit unsupported Bulk API v2 query/ingest job routes, composite sObject insert plus explicit unsupported generic composite/batch/tree/graph responses, normal REST JSON payloads, Salesforce-shaped error arrays, SQLite persistence, and fixture/scoped reset endpoints are wired with black-box compatibility coverage. Full auth, durable query locators, full layout UI metadata, Tooling object coverage, modeled generic Composite orchestration, modeled Bulk API jobs, and broader REST resources remain incomplete.
