...
- Status: `partial`
- Gap: Apex insert/update/delete/upsert/undelete/merge syntax and Database.insert/update/delete/upsert/undelete/merge allOrNone paths now call the DML engine, return SaveResult/DeleteResult/UpsertResult/MergeResult/UndeleteResult/EmptyRecycleBinResult/LockResult/UnlockResult objects with single and multi-entry Database.Error lists carrying statusCode, message, fields arrays, and empty extended-error-detail lists, expose isCreated, merged record IDs, and updated related record IDs, set Ids, stamp common system fields, roll back allOrNone failures, soft-delete and undelete records, return structured active-row emptyRecycleBin errors, match implicit and explicit external-ID upserts, reject ID/object mismatches, reject formula-backed calculated field writes, enforce unique fields, validate lookup references, enforce simple Metadata API validation rules, reparent lookups on merge, fire supported upsert insert/update and merge update/delete trigger hooks only for successful rows, cascade soft-delete children from relationship metadata, and has bounded bulk-DML stress coverage. Trigger context includes operation flags, size, new/old lists, nullable unavailable contexts, and newMap/oldMap for supported operations. Complex validation-rule formulas, remaining undelete edge cases, richer duplicate/extended error details, and full Salesforce status-code parity remain incomplete.
...
- Status: `partial`
- Gap: Root API version discovery, CRUD/query/queryAll including bounded local batchSize pagination, REST queryMore locators, and Tooling query/queryAll continuation paths, explicit unsupported SOQL query plan explain probes, SObject discovery and object-resource metadata with stable describe/recent/updated/deleted URL shapes, describe payloads with conservative object, field, record type, picklist, and child relationship metadata, describe/recent, conservative updated/deleted resources backed by local system timestamps and soft-delete flags, record GET attributes/url payloads, conservative SObject external-ID GET/PATCH upsert/DELETE routes backed by local DML behavior, compact layout stubs including describe aliases and explicit unsupported SObject layout-adjacent approval/named-layout metadata routes, empty SObject list-view collection stubs plus explicit unsupported describe/results routes, explicit unsupported SObject object-scoped quick action collection/detail/default-values routes, row-template placeholder malformed-ID errors, a deterministic /limits payload with common Max/Remaining entries, deterministic local OAuth userinfo/id stubs with conservative OpenID URL/profile fields plus explicit unsupported OAuth token/revoke/introspect/authorize probes with X-OAER-User-Id and Authorization: Bearer <userId> selection, Tooling root discovery, Tooling executeAnonymous with selected user context, strict-limit mode, successful DML commit, and runtime-error/limit rollback evidence, Tooling query/queryAll with Tooling queryMore continuation paths, explicit unsupported Tooling sObject/completions routes, stable unsupported Tooling metadata/debugger/log/test object, test-run, and coverage probes, Bulk Jobs root discovery plus explicit unsupported Bulk API v2 query/ingest job and common job subroute probes with pinned result/locator error bodies, composite sObject insert/update/delete collections with allOrNone rollback plus conservative typed composite sObject external-ID upsert backed by local DML with all-or-none rollback and reference IDs, and explicit unsupported generic composite/batch/tree/graph responses with route-specific envelope validation and malformed JSON edge envelopes, explicit unsupported common REST namespace stubs for Connect/Chatter/Analytics/Wave/Metadata/Support/Process/Actions/Apps/AppMenu/Tabs/Theme/QuickActions with matching resource-discovery links, normal REST JSON payloads, Salesforce-shaped error arrays, SQLite persistence, and fixture/scoped reset endpoints are wired with black-box compatibility coverage. Full OAuth security/token issuance, durable query locators, full layout UI metadata, modeled SObject list view describe/results, modeled Tooling records, modeled Tooling test orchestration and code coverage, modeled generic Composite orchestration, modeled Bulk API jobs and real Bulk result streams, and deeper REST resources remain incomplete.
