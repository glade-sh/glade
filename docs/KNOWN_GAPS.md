...
- Status: `partial`
- Gap: CRUD/query/queryAll including bounded local batchSize pagination and process-local queryMore locators, SObject discovery and object-resource metadata with stable describe/recent URL shapes, describe/recent, record GET attributes/url payloads, explicit layout and default-values unsupported responses, compact layout stubs, row-template placeholder malformed-ID errors, limits, deterministic local OAuth userinfo/id stubs with X-OAER-User-Id and Authorization: Bearer <userId> selection, Tooling executeAnonymous with selected user context, strict-limit mode, successful DML commit, and runtime-error/limit rollback evidence, Tooling query, explicit unsupported Tooling sObject/completions routes, stable unsupported Tooling metadata object/test-run/coverage probes, explicit unsupported Bulk API v2 query/ingest job routes, composite sObject insert plus explicit unsupported composite sObject update/delete/upsert route boundaries and generic composite/batch/tree/graph responses, explicit unsupported common REST namespace stubs for Connect/Chatter/Analytics/Wave/Metadata/Support/Process/Actions/Apps, normal REST JSON payloads, Salesforce-shaped error arrays, SQLite persistence, and fixture/scoped reset endpoints are wired with black-box compatibility coverage. Full auth, durable query locators, full layout UI metadata, Tooling object coverage, modeled Tooling test orchestration and code coverage, modeled generic Composite orchestration, modeled Bulk API jobs, and deeper REST resources remain incomplete.
