...
- Status: `partial`
- Gap: Root API version discovery, CRUD/query/queryAll including bounded local batchSize pagination, REST queryMore locators, and Tooling query/queryAll continuation paths, explicit unsupported SOQL query plan explain probes, SObject discovery and object-resource metadata with stable describe/recent/updated/deleted URL shapes, describe payloads with conservative object, field, record type, picklist, and child relationship metadata, describe/recent, conservative updated/deleted resources backed by local system timestamps and soft-delete flags, record GET attributes/url payloads, conservative SObject external-ID GET/PATCH upsert/DELETE routes backed by local DML behavior, compact layout stubs including describe aliases, source-backed list view, layout, compact-layout, Tooling, and Metadata read records, explicit unsupported SObject object-scoped quick action collection/detail/default-values routes, row-template placeholder malformed-ID errors, a deterministic /limits payload with common Max/Remaining entries, deterministic local OAuth userinfo/id stubs with conservative OpenID URL/profile fields plus explicit unsupported OAuth token/revoke/introspect/authorize probes with X-OAER-User-Id and Authorization: Bearer <userId> selection, Tooling executeAnonymous with selected user context, strict-limit mode, successful DML commit, runtime-error/limit rollback evidence, local StaticResourceCalloutMock and MultiStaticResourceCalloutMock behavior, local Apex @RestResource dispatch with RestContext setup and Salesforce-shaped unsupported-signature errors, generic Composite orchestration with reference substitution and all-or-none rollback for supported local subroutes, EventBus.publish local no-op success records, server executeAnonymous async queue drain, narrow ConnectApi organization settings stubs, explicit unsupported common REST namespace stubs for Connect/Chatter/Analytics/Wave/Support/Process/Actions/Apps/AppMenu/Tabs/Theme/QuickActions with matching resource-discovery links, normal REST JSON payloads, Salesforce-shaped error arrays, SQLite persistence, fixture/scoped reset endpoints, and the server example harness are wired with black-box compatibility coverage. Full OAuth security/token issuance, durable query locators, full layout UI metadata, Tooling writes/deploy/test orchestration and code coverage, Composite batch/tree/graph, Bulk API jobs and real Bulk result streams, platform event delivery, Visualforce rendering, broader ConnectApi behavior, deeper REST resources, and green example-project evidence remain incomplete.
