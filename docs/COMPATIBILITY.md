...
| SObject/schema runtime | partial | Runtime SObject values preserve projected fields and explicit nulls; schema describe registry and deterministic key prefixes are available. Apex construction, typed/dynamic field access, `get`/`put` with previous-value return, `isSet`, `clear`, `getPopulatedFieldsAsMap`, object-level and field-level `addError`, `hasErrors`/`getErrors`, DML Id propagation, and simple parent relationship projection are wired into the VM. Broader describe/system fields remain incomplete. |
| SOQL | partial | Static SOQL and `Database.query` execute in-memory `SELECT fields FROM Object` queries with binds, dynamic binds beside operators, dotted bind paths, collection binds, projection, `FIELDS(ALL/STANDARD/CUSTOM)`, `TYPEOF` relationship projection, equality/inequality/comparison filters, `AND`/`OR`, `IN`/`NOT IN`, `LIKE`, `NOT`, parenthesized conditions, common date literals, semi-joins, anti-joins, child relationship subqueries, soft-deleted row visibility through `ALL ROWS`, comma-separated `ORDER BY ASC/DESC` with `NULLS FIRST/LAST`, `LIMIT`, `OFFSET`, `FOR UPDATE`, `WITH SECURITY_ENFORCED`, `WITH USER_MODE`, `WITH SYSTEM_MODE`, `COUNT()`, `COUNT(field)`, `COUNT_DISTINCT`, `SUM`, `MIN`, `MAX`, `AVG`, `GROUP BY`, `ROLLUP`, `CUBE`, `HAVING` on aggregate expressions, aggregate aliases, `GROUPING(field)`, `AggregateResult` `exprN` fields, single-SObject assignment, catchable `QueryException` parse errors, and simple parent relationship fields. SQLite planning, lock contention, security enforcement, and advanced relationship query behavior are not complete. |
| DML/transactions/triggers | partial | `internal/dml` and the VM support Apex insert/update/delete/upsert/undelete/merge syntax, `Database.*` allOrNone results, `SaveResult`/`UpsertResult`/`MergeResult` objects with structured `Database.Error` values, required/unknown field validation, deterministic IDs, soft delete/undelete query visibility, implicit and explicit external-ID upsert, ID/object mismatch checks, unique fields, lookup reference validation, merge loser soft-delete with child lookup reparenting, cascade soft delete from relationship metadata, rollback snapshots, and before/after trigger invocation with operation flags, size, new/old lists, nullable unavailable contexts, newMap/oldMap, and object-level and field-level `addError` row failures. Validation-rule formulas, full merge trigger semantics, and complete bulk ordering remain incomplete. |
