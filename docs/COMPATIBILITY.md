...
| Local API server | partial | `oaer server` starts a Salesforce-shaped REST baseline with data version, SObject CRUD, normal REST JSON payload decoding, query/queryAll, describe/recent, limits, OAuth userinfo/id stubs, Tooling `executeAnonymous` with `--limit-mode`, composite sObject insert, optional SQLite persistence through `--db`, OAER fixture/reset endpoints, and Salesforce-shaped error arrays. Full auth, Tooling object coverage, Composite Graph, Bulk API, and broader REST resources are not complete. |
