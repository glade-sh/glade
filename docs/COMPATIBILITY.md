...
| SObject/schema runtime | partial | Runtime SObject values preserve projected fields and explicit nulls; schema describe registry and deterministic key prefixes are available. Apex construction, typed/dynamic field access, object-level and field-level `addError`, `hasErrors`/`getErrors`, DML Id propagation, and simple parent relationship projection are wired into the VM. Broader describe/system fields remain incomplete. |
| SOQL | partial | Static SOQL and `Database.query` execute in-memory `SELECT fields FROM Object` queries with binds, projection, equality/inequality/comparison filters, `AND`/`OR`, `IN`/`NOT IN`, `LIKE`, `NOT`, parenthesized conditions, common date literals, semi-joins, anti-joins, child relationship subqueries, comma-separated `ORDER BY ASC/DESC` with `NULLS FIRST/LAST`, `LIMIT`, `OFFSET`, `COUNT()`, `COUNT(field)`, `COUNT_DISTINCT`, `SUM`, `MIN`, `MAX`, `AVG`, `GROUP BY`, `ROLLUP`, `CUBE`, `HAVING` on aggregate expressions, aggregate aliases, `GROUPING(field)`, `AggregateResult` `exprN` fields, single-SObject assignment, and simple parent relationship fields. SQLite planning and advanced relationship query behavior are not complete. |
| DML/transactions/triggers | partial | `internal/dml` and the VM support Apex insert/update/delete/upsert/undelete syntax, `Database.*` allOrNone results, `SaveResult`/`UpsertResult` objects with structured `Database.Error` values, required/unknown field validation, deterministic IDs, soft delete/undelete query visibility, implicit and explicit external-ID upsert, ID/object mismatch checks, unique fields, lookup reference validation, cascade soft delete from relationship metadata, rollback snapshots, and before/after trigger invocation with `Trigger.*` basics plus object-level and field-level `addError` row failures. Merge, validation-rule formulas, and complete bulk ordering remain incomplete. |
