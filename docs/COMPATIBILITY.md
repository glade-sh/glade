...
| Watch mode | partial | `oaer test --watch` supports native `fsnotify` watching with polling fallback, debounce, JSON event stream with backend and run IDs, affected-test selection, reruns, and context cancellation. In-flight VM cancellation is not complete. |
