...
| LSP | partial | `oaer lsp` runs a stdio LSP transport for initialize/shutdown, diagnostics payloads, incremental open-buffer document sync, symbols, hover, and top-level Apex/SObject completion from the project index. Definition, references, semantic tokens, richer completion, and full check-parity diagnostics are not complete. |
