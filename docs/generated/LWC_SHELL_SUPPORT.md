...
| --- | --- | --- | --- | --- |
| `lwc.host.lightning-shell` | `lightning-shell` | supported-local | `/path/to/lwc-shell-local-browser.json#/support/lwc.host.lightning-shell/lightning-shell` | Host lane has passing local browser evidence, including the `/` and `/lwc` home routes, `/lwc/builder` draft composer, formal tab links, filterable available-LWC catalog, generated route list, and local preview routes; live Salesforce parity remains target-specific. |
