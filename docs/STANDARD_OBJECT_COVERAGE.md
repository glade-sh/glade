# Standard Object Coverage

Generated from `internal/storage` standard object metadata.

The describe baseline below is enriched at runtime by the generated SObject stub
field overlay in
`internal/storage/standard_sobject_stub_overlay_generated.go`.

Overlay summary:

- Overlay objects: 1,373
- Overlay fields: 25,075
- Overlay reference fields: 5,130
- Generator:
  `node scripts/generate-sobject-stub-overlay.mjs /path/to/fulgor/stubs/apex-sobject-stubs internal/storage/standard_sobject_stub_overlay_generated.go`

Describe baseline summary:

- Objects: 40
- Key prefixes: 37
- Fields: 2395
- Relationships: 197
- Record types: 16
- Picklist fields: 183
- Reference fields: 197

| Object | Key Prefix | Fields | Relationships | Record Types | Picklists | References |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `Account` | `001` | 54 | 7 | 1 | 10 | 7 |
| `Asset` | `02i` | 43 | 10 | 1 | 5 | 10 |
| `Attachment` | `00P` | 16 | 4 | 0 | 1 | 4 |
| `Campaign` | `701` | 33 | 5 | 1 | 2 | 5 |
| `CampaignMember` | `00v` | 36 | 7 | 1 | 3 | 7 |
| `Case` | `500` | 50 | 12 | 1 | 6 | 12 |
| `Contact` | `003` | 75 | 7 | 1 | 13 | 7 |
| `ContentDocument` | `069` | 30 | 7 | 0 | 5 | 7 |
| `ContentDocumentLink` | `06A` | 7 | 2 | 0 | 2 | 2 |
| `ContentVersion` | `068` | 48 | 9 | 1 | 7 | 9 |
| `Contract` | `800` | 52 | 7 | 1 | 10 | 7 |
| `Document` | `015` | 24 | 4 | 0 | 0 | 4 |
| `EmailTemplate` | `00X` | 29 | 6 | 0 | 5 | 6 |
| `EntityDefinition` | `` | 7 | 0 | 0 | 0 | 0 |
| `Event` | `00U` | 52 | 8 | 1 | 10 | 8 |
| `FieldDefinition` | `` | 7 | 1 | 0 | 0 | 1 |
| `FieldPermissions` | `0FP` | 1 | 0 | 0 | 0 | 0 |
| `Group` | `00G` | 4 | 0 | 0 | 0 | 0 |
| `KnowledgeArticleVersion` | `` | 16 | 2 | 0 | 0 | 2 |
| `Lead` | `00Q` | 55 | 8 | 1 | 9 | 8 |
| `ObjectPermissions` | `110` | 1 | 0 | 0 | 0 | 0 |
| `Opportunity` | `006` | 41 | 11 | 1 | 6 | 11 |
| `OpportunityContactRole` | `00K` | 12 | 4 | 0 | 2 | 4 |
| `OpportunityLineItem` | `00k` | 7 | 3 | 0 | 0 | 3 |
| `Order` | `801` | 69 | 12 | 1 | 11 | 12 |
| `OrderItem` | `802` | 40 | 8 | 0 | 4 | 8 |
| `Organization` | `00D` | 56 | 2 | 0 | 16 | 2 |
| `PermissionSet` | `0PS` | 562 | 5 | 0 | 1 | 5 |
| `PermissionSetAssignment` | `0Pa` | 8 | 3 | 0 | 0 | 3 |
| `Pricebook2` | `01s` | 14 | 2 | 1 | 0 | 2 |
| `PricebookEntry` | `01u` | 7 | 2 | 0 | 0 | 2 |
| `Product2` | `01t` | 28 | 3 | 1 | 8 | 3 |
| `Profile` | `00e` | 556 | 3 | 0 | 1 | 3 |
| `Quote` | `0Q0` | 81 | 8 | 1 | 14 | 8 |
| `RecordType` | `012` | 14 | 3 | 0 | 1 | 3 |
| `SetupEntityAccess` | `0J0` | 1 | 0 | 0 | 0 | 0 |
| `Task` | `00T` | 42 | 7 | 1 | 11 | 7 |
| `User` | `005` | 199 | 10 | 0 | 15 | 10 |
| `UserLicense` | `100` | 1 | 0 | 0 | 0 | 0 |
| `UserRole` | `00E` | 17 | 5 | 0 | 5 | 5 |
